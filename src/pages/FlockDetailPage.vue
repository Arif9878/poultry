...
const { flock, recentLogs, trend, kpis, loading, error, loadFlockDetail } =
  useFlocks();
    ...
    <template v-else-if="flock">
        ...
        <MetricCard
          ...
          :value="
            flock.flock_type === 'layer'
              ? `${formatDecimal(kpis.henDayProduction)}%`
              : `${formatDecimal(kpis.averageSampleWeight)} g`
          "
          :helper="
            flock.flock_type === 'layer'
              ? 'Produksi telur dibanding populasi hidup'
            ...
            >
              ...
              <div class="mt-3 grid gap-3 sm:grid-cols-3">
                <div>
                  ...
                  <p class="font-semibold text-ink">
                    {{ formatDecimal(item.feed_used_kg) }} kg
                  </p>
                </div>
                <div>
      ...
      <section class="space-y-4">
        <div>
          ...
          <p class="text-sm text-slate-500">
            Log terakhir membantu cek kualitas data dan status sync.
          </p>
        </div>

        <div v-if="recentLogs.length" class="space-y-3">
          <article v-for="log in recentLogs" :key="log.id" class="surface-card">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-4">
              <div class="rounded-2xl bg-slate-50 p-3">
                <p class="text-xs text-slate-500">Feed</p>
                <p class="font-semibold text-ink">
                  {{ formatDecimal(log.feed_used_kg) }} kg
                </p>
              </div>
              <div class="rounded-2xl bg-slate-50 p-3">
                <p class="text-xs text-slate-500">Ayam mati</p>
              ...
              <div class="rounded-2xl bg-slate-50 p-3">
                ...
                <p class="font-semibold text-ink">
                  {{ formatNumber(log.live_population) }}
                </p>
              </div>
              <div class="rounded-2xl bg-slate-50 p-3">
                <p class="text-xs text-slate-500">
